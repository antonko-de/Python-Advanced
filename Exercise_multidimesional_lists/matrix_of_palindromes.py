...
last_char, middle_char = list(map(int, input().split()))

matrix = []

for row in range(last_char):
    symbol = chr(FIRST_CHAR+row)
    curr = [symbol + chr(ord(symbol)+i) + symbol for i in range(middle_char)]
    matrix.append(curr)
