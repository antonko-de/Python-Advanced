'''Write a program that evaluates a string expression. You will be given that string expression on the first line in the form of 
numbers and operators separated with a single space from each other. Your job is to take that string expression and calculate the result after evaluating it.
...
we again take all the numbers we have (8, 1, 3) and apply the operator to them in that order (8 - 1 - 3 = 4). In the end, we print the result.
...
while input_string:
    curr = input_string.popleft()
    to_calculate.append(curr)
    
    if curr in '*/+-':
        curr_result = str(int(eval(f'{curr}'.join(to_calculate)[:-2:])))
        to_calculate = deque([curr_result])
        

print(int(float(to_calculate.pop())))
